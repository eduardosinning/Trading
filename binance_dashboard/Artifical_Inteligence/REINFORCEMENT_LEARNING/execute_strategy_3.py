...
def main() -> None:
    ...
    try:
        #single_asset = ['BTC','ETH','USDT']
        single_asset = False
        bot = TradingBot(single_asset=single_asset, simulation_mode=True, estrategia='strategy3')
        
        if not bot.agents:
            logging.error("No se pudo configurar ningún agente de trading")
            return
        
        bot.train_agent(timesteps=1000)
            
        target_profit = 0.20
        history_file = 'strategy_3_trading_history.json'

        # Inicializar archivo de historial vacío
        with open(history_file, 'w') as f:
            json.dump([], f)

        logging.info("Iniciando ejecución de estrategia 2...")
        ...
        try:
            ...
            while running:
                ...
                try:
                    ...
                    logging.info(f"Retorno estrategia 2: {total_profit_usd:.8f} USD, {avg_profit_pct:.2f}%, "
